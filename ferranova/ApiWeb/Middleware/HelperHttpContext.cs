        ...
        private TokenClaims getTokenClaims(HttpContext request)
        {
            ...
            if (autorizacion != null)
            {
                ...
                if (identity != null)
                {
                    ...
                    obj.RoleName = identity.Claims.Where(c => c.Type == "RoleName").Select(c => c.Value).SingleOrDefault();
                    obj.IdPersona = int.Parse(identity.Claims.Where(c => c.Type == "IdPersona").Select(c => c.Value).SingleOrDefault());
